...
class Lattice:
    ...
    @methods:
        - is_unit_cell: Returns whether a given lattice is a unit cell
        - init_voxels: Initializes Voxel objects and their coordinates
    ...
    def __init__(self, input_lattice: np.array):
        ...
        is_unit_cell = Lattice.is_unit_cell(input_lattice)
    ...
    def get_voxel(self, voxel_id) -> Voxel:
        ...
        @param:
            - voxel_id: int (Voxel.index) or tuple (Voxel.coordinates)
        @return:
            - voxel: Voxel object
        """
        if isinstance(voxel_id, int):
            ...
            raise ValueError("voxel_id must be an int (index) or a tuple/np.array (coordinates)")
    ...
    def get_partner(self, voxel, direction: tuple) -> tuple[Voxel, Vertex]:
        ...
        for dim_index, coords in enumerate(partner_coords):
            ...
            LOWER_BOUND = 0
            UPPER_BOUND = self.MinDesign.shape[dim_index]
